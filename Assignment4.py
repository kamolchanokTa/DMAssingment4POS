...
#Calc the probs for start word
start_p = {}
first_obs = obs[0]
for state in states:
    if not first_obs in obs_p[state]:
        start_p[state] = 0
    else:
        start_p[state] = obs_p[state][first_obs]

for state in states:
    start_p[state] = start_p[state] / sum(start_p.values())

#Set dictionary's value 0 if they do not exist in the input.
for ob in obs:
    for state in states:
